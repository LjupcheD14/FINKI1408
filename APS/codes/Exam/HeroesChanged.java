// Instead of middle element, last element
...
class SLL<E> {
    ...
    public void setLast(SLLNode<E> last) {
...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        ...
        SLLNode<Card> max = firstFriendCards.getFirst();

        while (node != null) {
            if (max.element.damage() < node.element.damage()) {
                max = node;
            }
            node = node.succ;
        }

        firstFriendCards.delete(max);


        int i = 0;
        SLLNode<Card> node2 = secondFriendCards.getFirst();

        while (i < 5 && node2 != null) {
            node2 = node2.succ;
            i++;
        }

        if (node2 != null) {
            SLLNode<Card> node3 = node2.succ;
            node2.succ = max;
            max.succ = node3;
        } else {
            secondFriendCards.setLast(max);
            max.succ = null;
        }
