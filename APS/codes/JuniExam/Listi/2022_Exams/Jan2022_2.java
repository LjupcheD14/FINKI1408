...
public class Jan2022_2 {

    // TODO: implement function
    public static void forum(SLL<Discussion> health, SLL<Discussion> finance) {
        SLLNode<Discussion> node1 = health.getFirst();
        SLLNode<Discussion> tmp1 = null;
        int max = 0;

        while (node1 != null) {
            int importance = 1;
            importance = node1.element.getPopularity() * node1.element.getUsers();
            if (importance > max) {
                max = importance;
                tmp1 = node1;
            }
        ...
        finance.insertLast(tmp1.element);
        
        SLLNode<Discussion> node2 = finance.getFirst();
        SLLNode<Discussion> tmp2 = null;

        int min = 999;
        while (node2 != null) {
            int importance = 0;
            importance = node2.element.getPopularity() * node2.element.getUsers();
            if (importance < min) {
                min = importance;
                tmp2 = node2;
            }
