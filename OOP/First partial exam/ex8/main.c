...
struct ITStore
{
    char store_name[100], location[100];
    Laptop l_arr[100];
    int n;
};
...
void print(ITStore *arr, int n)
{
    for(int i=0; i<n; i++)
    {
        ...
        {
            float s_s = arr[i].l_arr[j].screen_size_in;
            printf("%s ",arr[i].l_arr[j].manufacturer);
            (s_s-(int)s_s)==0?printf("%d",(int)s_s):printf("%.1f",s_s);
            printf(" %d\n",arr[i].l_arr[j].price);
        }
    }
}
...
void best_offer(ITStore *arr, int n)
{
    ...
    for(int i=0; i<n; i++)
    {
        for(int j=0; j<arr[i].n; j++)
        {
            if(arr[i].l_arr[j].price<min && arr[i].l_arr[j].touch==1)
            {
                min = arr[i].l_arr[j].price;
                pos_i=i,pos_j=j;
            }
        }
    }
