...
protected:
    ...
    int visina;
public:
    Image(const char *ime="untitled", const char *korisnik="unknown", int sirina=800, int visina=800)
    {
...
private:
    bool trans;
public:
    TransparentImage(const char *ime="untitled", const char *korisnik="unknown", int sirina=800, int visina=800, bool trans=false) : Image(ime,korisnik,sirina,visina)
    {
        this->trans=trans;
    }
    double fileSize()
...
Folder max_folder_size(Folder *folders, int n)
{
    ...
    for(int i=1; i<n; i++)
    {
        if(folders[i].folderSize()>tmp.folderSize())
        {
            tmp=folders[i];
        }
    }
